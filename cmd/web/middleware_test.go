...
// Test that the secureHeaders middleware is correctly setting the header
func TestSecureHeaders(t *testing.T) {
	// This test can be run in parallel with other tests acknowledge to be able
	// to run in parallel.
	t.Parallel()
	...
	if err != nil {
		t.Fatal(err) // http.Request could not be set, fail catastrophically
	}
